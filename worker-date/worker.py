...
logger = logging.getLogger('date-worker-logger')
logger.setLevel(logging.INFO)
logger.addHandler(logging.StreamHandler())

rabbitmq_indices = RabbitQueue(
    get_rabbit_settings().date_filter_queue
)
...
def handler(
    ...
    try:
        ...
        logger.info('Processing | date_parser | %s', json.dumps(data))
        start_time = time.time()
        result = llm.process_query(data.query)
        logger.info(
            '(%(seconds).3fsec)\nResult | date_parser | %(result)s',
            {
        ...
        redis_cli.put_cache(
            properties.headers['Task-Id'] + '_date',
