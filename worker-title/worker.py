...
logger = logging.getLogger('title-worker-logger')
logger.setLevel(logging.INFO)
logger.addHandler(logging.StreamHandler())

rabbitmq_indices = RabbitQueue(
    get_rabbit_settings().title_text_queue
)
...
def handler(
    ...
    try:
        ...
        logger.info('Processing | title_parser | %s', json.dumps(data))
        start_time = time.time()
        result = llm.process_query(data.query)
        logger.info(
            '(%(seconds).3fsec)\nResult | title_parser | %(result)s',
            {
        ...
        redis_cli.put_cache(
            properties.headers['Task-Id'] + '_title',
